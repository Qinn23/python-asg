...
class PomodoroTimer:
    def __init__(self, root):
        ...
        self.current_task = ""
        self.cat_state = "sleeping"  # or "awake"
    ...
    def setup_ui(self):
        ...
        tk.Label(coin_frame, text=f"Coins: {self.coins}", font=('Arial', 14), bg='#f5f5f5').pack(side=tk.LEFT, padx=10)
    ...
    def draw_cat(self, state):
        """Draw the cat in the specified state (sleeping or awake)"""
        self.cat_canvas.delete("all")
        if state == "sleeping":
            # Draw sleeping cat
            self.cat_canvas.create_oval(50, 50, 150, 150, fill='#ffcc99', outline='#d4b483')
            self.cat_canvas.create_line(75, 75, 45, 65, width=2)  # Left eye closed
            self.cat_canvas.create_line(125, 75, 155, 65, width=2)  # Right eye closed
            self.cat_canvas.create_arc(75, 90, 125, 120, start=0, extent=180, style=tk.CHORD, fill='#ff6666')  # Sleeping mouth
        else:
            # Draw awake cat
            self.cat_canvas.create_oval(50, 50, 150, 150, fill='#ffcc99', outline='#d4b483')
            self.cat_canvas.create_oval(70, 70, 90, 90, fill='black')  # Left eye
            self.cat_canvas.create_oval(110, 70, 130, 90, fill='black')  # Right eye
            self.cat_canvas.create_arc(85, 100, 115, 130, start=0, extent=-180, style=tk.ARC, width=2)  # Mouth
            
            # Ears
            self.cat_canvas.create_polygon(60, 40, 40, 20, 80, 50, fill='#ffcc99', outline='#d4b483')  # Left ear
            self.cat_canvas.create_polygon(140, 40, 160, 20, 120, 50, fill='#ffcc99', outline='#d4b483')  # Right ear
    
    def set_task(self):
        """Prompt user to set the task"""
        task = tk.simpledialog.askstring("Set Task", "What would you like to focus on this session?")
        if task:
    ...
    def timer_complete(self):
        ...
        if self.is_focus:
            ...
            # Award coins
            coins_earned = random.randint(1, 3)  # Random coins between 1-3
            ...
            if self.pomodoro_count % 4 == 0:
                ...
                messagebox.showinfo("Long Break", f"You've completed 4 Pomodoros! Take a long break of {self.settings['long_break_time']} minutes.")
            else:
                self.is_focus = False
                self.remaining_time = self.settings['break_time'] * 60
    ...
    def next_phase(self):
        ...
        self.update_display()
        self.draw_cat("awake" if not self.is_focus else "sleeping")
    ...
    def save_timer_settings(self):
        ...
        try:
            ...
            if focus_time < 1 or focus_time > 99 or break_time < 1 or break_time > 99 or long_break < 1 or long_break > 99:
                raise ValueError("Times must be between 1-99 minutes")
    ...
    def set_sound(self, sound_type):
        """Set a sound file for the timer"""
    ...
    def connect_music_app(self):
        ...
        choice = messagebox.askyesno("Music Apps", 
                                   "Would you like to open Youtube music for your music?\n"
    ...
    def open_cat_shop(self):
        ...
        # Info label
        info_label = tk.Label(shop_window, text="Select an item to buy for your cat:", font=('Arial', 12))
        info_label.pack()
        ...
        for idx, item in enumerate(items):
            ...
            tk.Label(item_row, text=f"{item['icon']} {item['name']}: {item['price']} coins", width=25, anchor=tk.W).pack(side=tk.LEFT)
            
            btn = ttk.Button(item_row, text="Buy", 
                           command=lambda i=item: self.buy_item(i),
                           state=tk.NORMAL if self.coins >= i['price'] else tk.DISABLED)
            btn.pack(side=tk.RIGHT)
        
        close_button = ttk.Button(shop_window, text="Close", command=shop_window.destroy)
        close_button.pack(pady=10)
    
    def buy_item(self, item):
        """Buy an item for the cat"""
        if self.coins >= item['price']:
            self.coins -= item['price']
            self.settings['coins'] = self.coins
            
            # Add item to cat's possessions if not already there
            if 'cat_items' not in self.settings:
                self.settings['cat_items'] = []
            ...
            # Close and reopen shop to update UI
